...
- Параметр `title_font_size` в `_config.yml` позволяет принудительно
  задать размер шрифта для заголовка
- Параметр `mathjax` в `_config.yml` включает поддержку
  [MathJax](https://www.mathjax.org/) версии 3.  Использование косой
  черты требует дополнительного экранирования `\\[ \\]`.
- `include toc.html` для красивого оглавления содержимого страницы
  (`\{: .no_toc \}` для исключения заголовка из списка
